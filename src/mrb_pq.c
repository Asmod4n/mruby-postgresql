...
mrb_PQftable(mrb_state *mrb, mrb_value self)
{
  ...
  if (foo == InvalidOid) {
    mrb_raise(mrb, E_RUNTIME_ERROR, "InvalidOid");
  }
...
mrb_PQgetvalue(mrb_state *mrb, mrb_value self)
{
  ...
  if (value) {
    ...
    mrb_value value_str = mrb_str_new_static(mrb, value, PQgetlength(result, row_number, column_number));
    switch(PQftype(result, column_number)) {
      ...
      case 23:
      case 26: {
        return mrb_str_to_inum(mrb, value_str, 0, TRUE);
      } break;
      case 700:
      case 701: {
        return mrb_float_value(mrb, mrb_str_to_dbl(mrb, value_str, TRUE));
      } break;
      default: {
        mrb_str_modify(mrb, RSTRING(value_str));
        return value_str;
      }
    }
  } else {
